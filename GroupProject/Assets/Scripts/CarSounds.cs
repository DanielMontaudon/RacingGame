...
public class CarSounds : MonoBehaviour
{
    ...
    public float maxRolloffDistance = 5000;                                      // The maximum distance where rollof starts to take place
    ...
    [SerializeField] private GameObject m_CarObject; // Reference to car we are controlling
    ...
    private void Update()
    {
        ...
        float camDist = (Camera.main.transform.position - transform.position).magnitude;
        ...
        if (m_StartedSound)
        {
            ...
            Rigidbody rb = GetComponent<Rigidbody>();
            speed = rb.velocity.magnitude / 100;
            
            //speed = (transform.position - lastPosition).magnitude;
            //lastPosition = transform.position;

            float pitch = ULerp(lowPitchMin, lowPitchMax, speed);

            //Debug.Log(speed);

            // clamp to minimum pitch (note, not clamped to max for high revs while burning out)
            pitch = Mathf.Min(lowPitchMax, pitch);
            //Debug.Log("MIN VALUE: " + pitch);
            ...
            else
            {
                ...
                float highFade = Mathf.InverseLerp(0.2f, 0.8f, speed);
    ...
    // sets up audio source to the gane object
    private AudioSource SetUpEngineAudioSource(AudioClip clip)
    {
        // create the new audio clip component on the game object and set up its properties
